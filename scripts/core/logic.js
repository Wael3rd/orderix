...
function verifyOrder() {
    ...
    // --- DEBUT MODIFICATION DOBBLE ---
    if (currentDayConfig && GAME_MODES[currentDayConfig.modeId].isDobble) {
        if (selectionOrder.length !== 4) {
            resultDisplay.textContent = "Sélectionnez exactement 4 éléments (un par ligne).";
        ...
        if (isValidTarget && allMatch) {
            ...
            if (currentRound > totalRounds) {
                ...
                selectionOrder.forEach(item => { item.style.boxShadow = '0 0 0 4px #28a745'; item.style.transform = 'scale(1.1)'; });
                setTimeout(() => { if (!isPaused) window.startDobbleRound(); }, 500);
            }
        } else {
            selectionOrder.forEach(item => item.style.boxShadow = '0 0 0 4px #dc3545');
            showSolutionHighlight(window.validDobbleTargets);
            endGame('Erreur, mauvais symbole !', false);
        }
        return;
    }
    // --- FIN MODIFICATION DOBBLE ---
