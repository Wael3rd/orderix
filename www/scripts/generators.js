// ─── Génération des valeurs de jeu pour les 50 types ─────────────
// Correction d'audit : les valeurs étaient parfaitement déterministes
// (mêmes valeurs à chaque partie). Un jitter aléatoire est ajouté aux
// types continus, en préservant un écart perceptif minimal entre valeurs.
...
function generateValues(type, count) {
    ...
    if (FIXED[type]) {
        ...
        // Tirage aléatoire de `count` valeurs distinctes, triées (l'écart est garanti par la liste)
    ...
    for (let i = 0; i < count; i++) {
        let v = count === 1 ? range[0] : range[0] + i * step;
        // Jitter de ±25 % du pas : valeurs différentes à chaque partie,
        // ordre relatif et écart minimal (50 % du pas) préservés.
        if (count > 1) v += (Math.random() - 0.5) * step * 0.5;
        v = Math.min(range[1], Math.max(range[0], v));
        vals.push(isInt ? Math.round(v) : parseFloat(v.toFixed(4)));
    }
    if (isInt) {
        let seen = {}, u = [];
        for (let k = 0; k < vals.length; k++) { if (!seen[vals[k]]) { seen[vals[k]] = true; u.push(vals[k]); } }
        vals = u;
    }
    return vals;
}
