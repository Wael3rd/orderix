...
const GAME_MODES = {
    ...
    code: { rev: 2, name: "Le Code", isCode: true, typeAgnostic: true, desc: "Trouvez le code secret de 5 chiffres en 6 essais : après chaque essai, VERT = bien placé, OR = présent ailleurs, GRIS = absent. Façon Wordle !" },
    memoCroissant: { rev: 3, name: "Mémo Croissant", isMemoCroissant: true, typeAgnostic: true, desc: "Retournez deux cartes à la fois pour retrouver les paires, dans l'ordre : d'abord la paire 1, puis la 2, puis la 3… Sans pression : pas de vies, seul le chrono compte." },
    ...
    blocs: { rev: 3, name: "Blocs", isBlocs: true, typeAgnostic: true, desc: "GLISSEZ les pièces sur la grille : chaque ligne ou colonne complète s'efface. Effacez 6 lignes sans saturer la grille !" },
    dominosa: { rev: 1, name: "Les Dominos", isDominosa: true, typeAgnostic: true, desc: "Reliez les cases voisines deux par deux pour reconstituer les 10 dominos — chaque paire de chiffres n'existe qu'une seule fois. Déduction et annulation illimitée !" },
    ...
    swapSort: { rev: 2, name: "Échange Minimal", isSwapSort: true, typeAgnostic: true, desc: "Triez la colonne (petit en haut) en échangeant deux VOISINES à la fois — les échanges possibles s'illuminent. Budget limité !" },
    ...
    chronologie: { rev: 3, name: "Chronologie", isChronologie: true, typeAgnostic: true, desc: "Glissez les 8 cartes pour composer votre ordre, puis validez : les bonnes positions se verrouillent en vert. Recommencez jusqu'au sans-faute !" },
    ...
    piles: { rev: 2, name: "Les Piles", isPiles: true, typeAgnostic: true, desc: "Posez chaque pile de jetons sur l'un des 5 emplacements. Réunissez 7 jetons de même couleur d'affilée : la série s'envole ! 4 envols pour gagner, sans déborder." },
    ...
    nonogramme: { rev: 2, name: "Le Nonogramme", isNonogramme: true, typeAgnostic: true, desc: "Grille 7×7 : les chiffres indiquent les blocs de cases à remplir sur chaque ligne et colonne. Remplissez la grille — les indices passent au vert quand c'est juste." },
